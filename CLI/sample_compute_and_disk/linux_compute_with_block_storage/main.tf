...
module "disk01" {
  source     = "git::ssh://git@github.com/oracle-devrel/terraform-oci-cloudbricks-linux-iscsi-disks.git?ref=v1.0.2"
  ...
  iscsi_disk_instance_compartment_name = var.instance01_linux_compute_instance_compartment_name
  volume_display_name                  = var.disk01_volume_display_name
  backup_policy_level                  = var.disk01_backup_policy_level
  linux_compute_id                     = module.instance01.instance[count.index].id
