...
module "node02" {
  ...
  ######################################## PROVIDER SPECIFIC VARIABLES ######################################
  ######################################## ARTIFACT SPECIFIC VARIABLES ######################################
  ssh_public_key                      = var.node02_ssh_public_key
  ssh_private_key                     = var.node02_ssh_private_key
  ssh_public_is_path                  = var.node02_ssh_public_is_path
  ssh_private_is_path                 = var.node02_ssh_private_is_path
  oke_cluster_compartment_name        = var.node02_oke_cluster_compartment_name
  oke_network_compartment_name        = var.node02_oke_network_compartment_name
  oke_availability_domain_map         = var.node02_oke_availability_domain_map
  oke_cluster_name                    = var.node02_oke_cluster_name
  oke_nodepool_network_subnet_name    = var.node02_oke_nodepool_network_subnet_name
  k8s_version                         = var.node02_k8s_version
  node_pool_name                      = var.node02_node_pool_name
  node_pool_shape                     = var.node02_node_pool_shape
  is_flex_shape                       = var.node02_is_flex_shape
  nodepool_shape_config_ocpus         = var.node02_nodepool_shape_config_ocpus
  nodepool_shape_config_memory_in_gbs = var.node02_nodepool_shape_config_memory_in_gbs
  number_of_nodes                     = var.node02_number_of_nodes
  k8s_label_map                       = var.node02_k8s_label_map
  ######################################## ARTIFACT SPECIFIC VARIABLES ######################################
}